(function () {
  ...
  function initializeListeners() {
    function locatorMouseDownHandler() {
      elements.dragger.setSelected(true);
    }
    function bodyMouseUpHandler() {
      elements.dragger.setSelected(false);
    }
    function locatorMouseMoveHandler(event) {
      physics.setPosition(event.layerX);
    }
    function sliderLineMouseMoveHandler(event) {
      physics.setPosition(event.layerX);
    }
    elements.dragger.locator.addEventListener(
      "mousedown",
      locatorMouseDownHandler
    );
    document.body.addEventListener("mouseup", bodyMouseUpHandler);
    elements.dragger.locator.addEventListener(
      "mousemove",
      locatorMouseMoveHandler
    );
    elements.slider.line.addEventListener(
      "mousemove",
      sliderLineMouseMoveHandler
    );

    return function removeListeners() {
      elements.dragger.locator.removeEventListener(
        "mousedown",
        locatorMouseDownHandler
      );
      document.body.addEventListener("mouseup", bodyMouseUpHandler);
      ...
      elements.slider.line.removeEventListener(
        "mousemove",
        sliderLineMouseMoveHandler
      );
    };
  ...
  function updateSlider(state) {
    elements.filler.style.transform = `translate(${
      state.position - elements.slider.getSliderWidth()
    }px)`;
    elements.dragger.locator.style.transform = `translate(${
      state.position - 12
    }px , -50%)`; // dragger width is 24 px -> state.position - 12 = calc(state.position - 50%);
  ...
  function updateElementsState(variables) {
    var isPositionInRange =
      physics.currentPosition >= 0 &&
      physics.currentPosition <= elements.slider.getSliderWidth();
